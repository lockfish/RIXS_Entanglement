...
from witness_utils import diag, diag_evals_diff, diag_n_dipole
...
def QFI_denom_dimerCe(HKL=None, Gam_c=1., omega=None,
    ...
    denom = np.zeros((len(pol), len(omega), len(thin)), dtype=float)
    for nomega, om in enumerate(omega):
        for npol, (alpha, beta) in enumerate(pol):
            for nth, th in enumerate(thin):
                ...
                F_mag_real = (F_mag + F_mag.conj().T) * 0.5
                F_mag_imag = (F_mag - F_mag.conj().T) * (-0.5j)
                denom[npol,nomega,nth] += diag_evals_diff(F_mag_real) ** 2 + diag_evals_diff(F_mag_imag) ** 2
...
def conjugate_RIXS_dimerCe(HKL=None, Gam_c=1., sigma=0.1, fraction=0.5,
                           omega=None, eloss=None, eline=None,
                           phi=0., thin=15., tth=150., pol=[(0, 0), (0, np.pi / 2.0)],
                           T=10., tol=1e-5, ngs=None, folder=''):
    """
    Calculate the conjugate RIXS spectra for Ba3CeIr2O9.
    Eigen values and transition operators will be loaded from the folder.
    ...
    # Calculate conjugate RIXS
    F_fis = np.zeros((2, len(omega), 3, 3, len(gs), len(evals_i)), dtype=complex)
    for nomega, om in enumerate(omega):
        for nnTM, (evals_n, T_abs) in enumerate(zip(evals_n_all, T_abs_all)):
            T_emi = np.stack([np.conj(np.transpose(T0)) for T0 in T_abs])
            F_fis[nnTM,nomega,:,:,:,:] += scattering_mat(evals_i[:], evals_n[:], T_abs, T_emi[:, gs, :], om, Gam_c)
    ...
    for nomega, om in enumerate(omega):
        for npol, (alpha, beta) in enumerate(pol):
            for nth, th in enumerate(thin):
                ei0, ef0 = dipole_polvec_rixs(th/180.*np.pi, (-th+tth)/180.*np.pi, phi/180.*np.pi, alpha, beta)
                Q = np.dot(HKL[nth], Umat)
                F_mag = np.zeros((len(gs), len(evals_i)), dtype=complex)
                for nnTM in range(2):
                    ei = np.dot(ei0, rmat_t2o(nnTM+1))
                    ef = np.dot(ef0, rmat_t2o(nnTM+1))
                    phase = np.exp(-1j * np.dot(Q, pos[nnTM]))
                ...
                for nngs, gs_id in enumerate(gs):
                    for n in range(len(evals_i)):
                        if eline is None or evals_i[n]-evals_i[gs_id]>=eline:
                            Gaussian = (1 - fraction) / sigma_g / np.sqrt(2. * np.pi) * np.exp(-(eloss - (evals_i[n] - evals_i[gs_id]))**2 / 2. / sigma_g**2)
                            Lorentzian = fraction / np.pi * sigma / ((eloss - (evals_i[n] - evals_i[gs_id]))**2 + sigma**2)
                            rixs[npol,nomega,nth,:] += prob[nngs] * np.abs(F_mag[nngs, n])**2 * (Gaussian + Lorentzian)
    for nomega, om in enumerate(omega):
        for npol, (alpha, beta) in enumerate(pol):
            for nth, th in enumerate(thin):
                ei0, ef0 = dipole_polvec_rixs(th/180.*np.pi, (-th+tth)/180.*np.pi, phi/180.*np.pi, alpha, beta)
...
def calc_FQ_dimerCe(HKL=[0., 0., 0.], Gam_c=1., omega=0,
                    phi=0., thin=15., tth=150., pol=(0,0), folder=''):
    """
    Calculate the quantum Fisher information of pure states for Ba3CeIr2O9.
    Eigen values and transition operators will be loaded from the folder.
    ...
    states_64 = np.load(folder+'/states_64.npy')
    states_55 = np.load(folder+'/states_55.npy')
    states_46 = np.load(folder+'/states_46.npy')
    M_64 = states_64.conjugate() @ (evecs_i.T)
    M_55 = states_55.conjugate() @ (evecs_i.T)
    M_46 = states_46.conjugate() @ (evecs_i.T)
    ...
    F_fis = np.zeros((2, 3, 3, len(evals_i), len(evals_i)), dtype=complex)
    for nnTM, (evals_n, T_abs) in enumerate(zip(evals_n_all, T_abs_all)):
        T_emi = np.stack([np.conj(np.transpose(T0)) for T0 in T_abs])
        F_fis[nnTM,:,:,:,:] += scattering_mat(evals_i[:], evals_n[:], T_abs[:, :, :], T_emi, omega, Gam_c)
    
    alpha, beta = pol
    
    F_mag = np.zeros((len(evals_i), len(evals_i)), dtype=complex)
    for nnTM in range(2):
        ei0, ef0 = dipole_polvec_rixs(thin/180.*np.pi, (-thin+tth)/180.*np.pi, phi/180.*np.pi, alpha, beta)
        Q = np.dot(HKL, Umat)
        ei = np.dot(ei0, rmat_t2o(nnTM+1))
        ef = np.dot(ef0, rmat_t2o(nnTM+1))
        phase = np.exp(-1j * np.dot(Q, pos[nnTM]))
        for m in range(3):
            for n in range(3):
                F_mag[:, :] += ef[m] * F_fis[nnTM, m, n] * phase * ei[n]
    F_mag_real = (F_mag + F_mag.conj().T) * 0.5
    F_mag_imag = (F_mag - F_mag.conj().T) * (-0.5j)
    
    A_64_real = M_64 @ F_mag_real @ (M_64.conjugate().T)
    A_64_imag = M_64 @ F_mag_imag @ (M_64.conjugate().T)
    S_64_real = M_64 @ F_mag_real @ F_mag_real @ (M_64.conjugate().T)
    S_64_imag = M_64 @ F_mag_imag @ F_mag_imag @ (M_64.conjugate().T)

    A_55_real = M_55 @ F_mag_real @ (M_55.conjugate().T)
    A_55_imag = M_55 @ F_mag_imag @ (M_55.conjugate().T)
    S_55_real = M_55 @ F_mag_real @ F_mag_real @ (M_55.conjugate().T)
    S_55_imag = M_55 @ F_mag_imag @ F_mag_imag @ (M_55.conjugate().T)

    A_46_real = M_46 @ F_mag_real @ (M_46.conjugate().T)
    A_46_imag = M_46 @ F_mag_imag @ (M_46.conjugate().T)
    S_46_real = M_46 @ F_mag_real @ F_mag_real @ (M_46.conjugate().T)
    S_46_imag = M_46 @ F_mag_imag @ F_mag_imag @ (M_46.conjugate().T)

    def objective(params, A_r, S_r, A_i, S_i):
        n = len(params) // 2
        r = params[:n]
        theta = params[n:]
        z = r * np.exp(1j*theta)
        z = z.reshape(1,n)
        result = z.conjugate() @ S_r @ (z.T) + z.conjugate() @ S_i @ (z.T)
        result -= (z.conjugate() @ A_r @ (z.T)) ** 2 + (z.conjugate() @ A_i @ (z.T)) ** 2
        return -np.real(result * 4)

    def objective_64(params):
        return objective(params, A_64_real, S_64_real, A_64_imag, S_64_imag)
    def objective_55(params):
        return objective(params, A_55_real, S_55_real, A_55_imag, S_55_imag)
    def objective_46(params):
        return objective(params, A_46_real, S_46_real, A_46_imag, S_46_imag)
    ...
    optimal = []
    size = [15, 36, 15]
    func = [objective_64, objective_55, objective_46]
    for i in range(3):
        n = size[i]
        object_func = func[i]
        initial_r = np.full(n, 1/n)
        initial_theta = np.zeros(n)
        initial_guess = np.concatenate([initial_r, initial_theta])
        constraints = {'type': 'eq', 'fun': constraint}
        bounds = [(0,1)]*n + [(0, 2*np.pi)]*n
        result = minimize(object_func, initial_guess, constraints = [constraints], bounds=bounds)
        optimal.append(-result.fun)

    return max(optimal) * 2 # there are two dimers in a single unit
...
def QFI_denom_dimerCe_sum(HKL=None, Gam_c=1., omega=None,
                          phi=0., thin=15., tth=150., T=10., folder=''):
    """
    Calculate the QFI denominator for Ba3CeIr2O9.
    Eigen values and transition operators will be loaded from the folder.
...
def calc_RIXS_dimerCe_sum(HKL=None, Gam_c=1., sigma=0.1, fraction=0.5,
                          omega=None, eloss=None, eline=None,
                          phi=0., thin=15., tth=150., pol=[(0, 0), (0, np.pi / 2.0)],
                          T=10., tol=1e-5, ngs=None, folder=''):
    """
...
def conjugate_RIXS_dimerCe_sum(HKL=None, Gam_c=1., sigma=0.1, fraction=0.5,
                               omega=None, eloss=None, eline=None,
                               phi=0., thin=15., tth=150., pol=[(0, 0), (0, np.pi / 2.0)],
                               T=10., tol=1e-5, ngs=None, folder=''):
    """
...
def calc_FQ_dimerCe_sum(HKL=[0., 0., 0.], Gam_c=1., omega=0,
                        phi=0., thin=15., tth=150., pol=[(0,0), (0,np.pi/2.0)], folder=''):
    """
    Calculate the quantum Fisher information of pure states for Ba3CeIr2O9.
    Eigen values and transition operators will be loaded from the folder.
    ...
    for (alpha, beta) in pol:
        ...
        for nnTM in range(2):
            ...
            for m in range(3):
                for n in range(3):
                    F_mag[:, :] += ef[m] * F_fis[nnTM, m, n] * phase * ei[n]
        F_mag_real.append((F_mag + F_mag.conj().T) * 0.5)
        F_mag_imag.append((F_mag - F_mag.conj().T) * (-0.5j))
    ...
    for i in range(len(pol)):
        A_64_real.append(M_64 @ F_mag_real[i] @ (M_64.conjugate().T))
        A_64_imag.append(M_64 @ F_mag_imag[i] @ (M_64.conjugate().T))
        S_64_real.append(M_64 @ F_mag_real[i] @ F_mag_real[i] @ (M_64.conjugate().T))
        S_64_imag.append(M_64 @ F_mag_imag[i] @ F_mag_imag[i] @ (M_64.conjugate().T))

        A_55_real.append(M_55 @ F_mag_real[i] @ (M_55.conjugate().T))
        A_55_imag.append(M_55 @ F_mag_imag[i] @ (M_55.conjugate().T))
        S_55_real.append(M_55 @ F_mag_real[i] @ F_mag_real[i] @ (M_55.conjugate().T))
        S_55_imag.append(M_55 @ F_mag_imag[i] @ F_mag_imag[i] @ (M_55.conjugate().T))

        A_46_real.append(M_46 @ F_mag_real[i] @ (M_46.conjugate().T))
        A_46_imag.append(M_46 @ F_mag_imag[i] @ (M_46.conjugate().T))
        S_46_real.append(M_46 @ F_mag_real[i] @ F_mag_real[i] @ (M_46.conjugate().T))
        S_46_imag.append(M_46 @ F_mag_imag[i] @ F_mag_imag[i] @ (M_46.conjugate().T))
    ...
    def objective(params, A_r, S_r, A_i, S_i):
        ...
        for i in range(len(A_r)):
            ...
            z = r * np.exp(1j*theta)
            z = z.reshape(1,n)
            result += z.conjugate() @ S_r[i] @ (z.T) + z.conjugate() @ S_i[i] @ (z.T)
            result -= (z.conjugate() @ A_r[i] @ (z.T)) ** 2 + (z.conjugate() @ A_i[i] @ (z.T)) ** 2
        return -np.real(result * 4)
...
def Ta_ei_ef(tth, th, phi, chi):
    ...
    ei_TH  = [np.cos(np.deg2rad(tth/2)),  np.sin(np.deg2rad(tth/2)), 0.]
    ef1_TH = [np.cos(np.deg2rad(tth/2)), -np.sin(np.deg2rad(tth/2)), 0.]
    ef2_TH = [0., 0., 1.]
    ei = np.dot(tmat_TH2Q, ei_TH)
...
def Ta_ei_ef_sum(tth, th, phi, chi):
    UBmatrix = np.array([[-0.0935575 ,  1.04121448, -0.01640006],
                         [ 1.25487514,  0.70574976, -0.02770928],
                         [-0.07643596, -0.08424219, -0.43483902]])
    Bmatrix  = np.array([[ 1.09177851e+00,  6.30338615e-01, -1.33704305e-16],
...
def QFI_denom_dimerTa(HKL=None, Gam_c=0.6, omega=None,
    ...
    denom = np.zeros((2, len(omega), len(th)), dtype=float)
    for nomega, om in enumerate(omega):
        for nth, _ in enumerate(th):
            ei0, ef1, ef2 = Ta_ei_ef(tth[nth], th[nth], phi[nth], chi[nth])
            Q = np.dot(Bmat, HKL[nth])
            for npol, ef0 in enumerate([ef1,ef2]):
                F_mag = np.zeros((len(evals_i), len(evals_i)), dtype=complex)
                for nnTM in range(2):
                    ei = np.dot(ei0, rmat_t2o(nnTM+1))
                    ef = np.dot(ef0, rmat_t2o(nnTM+1))
...
def conjugate_RIXS_dimerTa(HKL=None, Gam_c=0.6, sigma0=0.1, sigmas=[0.05,0.05], fraction=0.5,
                           omega=None, eloss=None, eline=None,
                           chi=90., phi=0., th=15., tth=150.,
                           T=10., tol=1e-5, ngs=None, folder=''):
    """
                ...
                for nngs, gs_id in enumerate(gs):
                    for n in range(len(evals_i)):
                        ...
                        if eline is None or evals_i[n]-evals_i[gs_id]>=eline:
                            Gaussian = (1 - fraction) / sigma_g / np.sqrt(2. * np.pi) * np.exp(-(eloss - (evals_i[n] - evals_i[gs_id]))**2 / 2. / sigma_g**2)
                            Lorentzian = fraction / np.pi * sigma / ((eloss - (evals_i[n] - evals_i[gs_id]))**2 + sigma**2)
                            rixs[nef,nomega,nth,:] += prob[nngs] * np.abs(F_mag[nngs, n])**2 * (Gaussian + Lorentzian)
...
def calc_FQ_dimerTa(HKL=[0., 0., 0.], Gam_c=0.6, omega=0,
                    chi=90., phi=0., th=15., tth=150., folder=''):
    """
    Calculate the quantum Fisher information of pure states for Ba3TaIr2O9.
    Eigen values and transition operators will be loaded from the folder.
    ...
    evecs_i = np.load(folder+'/evecs_i.npy')
    states_65 = np.load(folder+'/states_65.npy')
    states_56 = np.load(folder+'/states_56.npy')
    M_65 = states_65.conjugate() @ (evecs_i.T)
    M_56 = states_56.conjugate() @ (evecs_i.T)
    ...
    ei0, ef1, ef2 = Ta_ei_ef(tth, th, phi, chi)
    Q = np.dot(Bmat, HKL)
    for npol, ef0 in enumerate([ef1,ef2]):
        F_mag = np.zeros((len(evals_i), len(evals_i)), dtype=complex)
        for nnTM in range(2):
    ...
    for i in range(2):
        A_65_real = M_65 @ F_mag_real[i] @ (M_65.conjugate().T)
        A_65_imag = M_65 @ F_mag_imag[i] @ (M_65.conjugate().T)
        S_65_real = M_65 @ F_mag_real[i] @ F_mag_real[i] @ (M_65.conjugate().T)
        S_65_imag = M_65 @ F_mag_imag[i] @ F_mag_imag[i] @ (M_65.conjugate().T)

        A_56_real = M_56 @ F_mag_real[i] @ (M_56.conjugate().T)
        A_56_imag = M_56 @ F_mag_imag[i] @ (M_56.conjugate().T)
        S_56_real = M_56 @ F_mag_real[i] @ F_mag_real[i] @ (M_56.conjugate().T)
        S_56_imag = M_56 @ F_mag_imag[i] @ F_mag_imag[i] @ (M_56.conjugate().T)

        def objective_65(params):
            return objective(params, A_65_real, S_65_real, A_65_imag, S_65_imag)
        def objective_56(params):
            return objective(params, A_56_real, S_56_real, A_56_imag, S_56_imag)

        optimal = []
        size = [6, 6]
        func = [objective_65, objective_56]
        for j in range(2):
            n = size[j]
            object_func = func[j]
            initial_r = np.full(n, 1/n)
        ...
        FQ.append(max(optimal) * 2) # there are two dimers in a single unit

    return FQ

# Sum

def QFI_denom_dimerTa_sum(HKL=None, Gam_c=0.6, omega=None,
                          chi=90., phi=0., th=15., tth=150.,
                          T=10., folder=''):
    """
    Calculate the QFI denominator for Ba3TaIr2O9.
    ...
    for nomega, om in enumerate(omega):
        for nth, _ in enumerate(th):
            ei1, ei2, ef1, ef2 = Ta_ei_ef_sum(tth[nth], th[nth], phi[nth], chi[nth])
...
def calc_RIXS_dimerTa_sum(HKL=None, Gam_c=0.6, sigma0=0.1, sigmas=[0.05,0.05], fraction=0.5,
                          omega=None, eloss=None, eline=None,
                          chi=90., phi=0., th=15., tth=150.,
                          T=10., tol=1e-5, ngs=None, folder=''):
    """
    ...
    for nomega, om in enumerate(omega):
        for nth, _ in enumerate(th):
            ei0, ei2, ef1, ef2 = Ta_ei_ef_sum(tth[nth], th[nth], phi[nth], chi[nth])
            Q = np.dot(Bmat, HKL[nth])
            for nef, ef0 in enumerate([ef1,ef2]):
                F_mag = np.zeros((len(evals_i), len(gs)), dtype=complex)
                for nnTM in range(2):
...
def conjugate_RIXS_dimerTa_sum(HKL=None, Gam_c=0.6, sigma0=0.1, sigmas=[0.05,0.05], fraction=0.5,
                               omega=None, eloss=None, eline=None,
                               chi=90., phi=0., th=15., tth=150.,
                               T=10., tol=1e-5, ngs=None, folder=''):
    """
    ...
    for nomega, om in enumerate(omega):
        for nth, _ in enumerate(th):
            ei1, ei2, ef0, ef2 = Ta_ei_ef_sum(tth[nth], th[nth], phi[nth], chi[nth])
            Q = np.dot(Bmat, HKL[nth])
            for nei, ei0 in enumerate([ei1,ei2]):
                F_mag = np.zeros((len(gs), len(evals_i)), dtype=complex)
                for nnTM in range(2):
                    ei = np.dot(ei0, rmat_t2o(nnTM+1))
                    ef = np.dot(ef0, rmat_t2o(nnTM+1))
                ...
                for nngs, gs_id in enumerate(gs):
                    for n in range(len(evals_i)):
                        ...
                        if eline is None or evals_i[n]-evals_i[gs_id]>=eline:
                            Gaussian = (1 - fraction) / sigma_g / np.sqrt(2. * np.pi) * np.exp(-(eloss - (evals_i[n] - evals_i[gs_id]))**2 / 2. / sigma_g**2)
                            Lorentzian = fraction / np.pi * sigma / ((eloss - (evals_i[n] - evals_i[gs_id]))**2 + sigma**2)
                            rixs[nei,nomega,nth,:] += prob[nngs] * np.abs(F_mag[nngs, n])**2 * (Gaussian + Lorentzian)
...
def calc_FQ_dimerTa_sum(HKL=[0., 0., 0.], Gam_c=0.6, omega=0,
                        chi=90., phi=0., th=15., tth=150., folder=''):
    """
    Calculate the quantum Fisher information of pure states for Ba3TaIr2O9.
    Eigen values and transition operators will be loaded from the folder.
    ...
    for i in range(2):
        A_65_real.append(M_65 @ F_mag_real[i] @ (M_65.conjugate().T))
        A_65_imag.append(M_65 @ F_mag_imag[i] @ (M_65.conjugate().T))
        S_65_real.append(M_65 @ F_mag_real[i] @ F_mag_real[i] @ (M_65.conjugate().T))
        S_65_imag.append(M_65 @ F_mag_imag[i] @ F_mag_imag[i] @ (M_65.conjugate().T))

        A_56_real.append(M_56 @ F_mag_real[i] @ (M_56.conjugate().T))
        A_56_imag.append(M_56 @ F_mag_imag[i] @ (M_56.conjugate().T))
        S_56_real.append(M_56 @ F_mag_real[i] @ F_mag_real[i] @ (M_56.conjugate().T))
        S_56_imag.append(M_56 @ F_mag_imag[i] @ F_mag_imag[i] @ (M_56.conjugate().T))
